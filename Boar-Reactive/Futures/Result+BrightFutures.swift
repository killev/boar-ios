...
extension ResultProtocol {
    ...
    public func flatMap<U>(_ f: (Value) -> Future<U>) -> Future<U> {
        return analysis(ifSuccess: {
            return f($0)
        }, ifFailure: {
            return Future<U>(error: $0)
        })
    }
}

extension ResultProtocol where Value: ResultProtocol {
    ...
    public func flatten() -> Result<Value.Value> {
...
extension ResultProtocol where Value: AsyncType, Value.Value: ResultProtocol {
