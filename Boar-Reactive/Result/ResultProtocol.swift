...
public extension ResultProtocol {
	...
	public func fanout<R: ResultProtocol>(_ other: @autoclosure () -> R) -> Result<(Value, R.Value)> {
    ...
    public func mapError<Error2: Error>(_ transform: (Error) -> Error2) -> Result<Value> {
		return flatMapError { Result<Value>.failure(transform($0)) }
	}
	...
	public func bimap<U>(success: (Value) -> U, failure: (Error) -> Error) -> Result<U> {
		return analysis(
