...
Element.DataSource: QueryableDataSourceProtocol, Element.DataSource.Index == Int{
...
public extension SignalProtocol where Element: ObservableArrayEventProtocol {
    ...
    @discardableResult
    public func bind(to tableView: NSTableView, animated: Bool = true, createCell: @escaping (Array<Element.Item>, Int, NSTableView) -> NSView?) -> Disposable {
        if animated {
            return toPatchesByResettingBatch()._bind(to: tableView, using: DefaultTableViewBond<Array<Element.Item>>(createCell: createCell))
        } else {
            return toPatchesByResettingBatch()._bind(to: tableView, using: ReloadingTableViewBond<Array<Element.Item>>(createCell: createCell))
        }
    }

    @discardableResult
    public func bind<B: TableViewBond>(to tableView: NSTableView, using bond: B) -> Disposable where B.DataSource == Array<Element.Item> {
...
public extension SignalProtocol where Element: ObservableArrayEventProtocol, Element.Item: Equatable{
    ...
    public func bind(to tableView: NSTableView, animated: Bool = true, createCell: @escaping (Array<Element.Item>, Int, NSTableView) -> NSView?) -> Disposable {
        if animated {
            return toPatches()._bind(to: tableView, using: DefaultTableViewBond<Array<Element.Item>>(createCell: createCell))
        } else {
            return toPatches()._bind(to: tableView, using: ReloadingTableViewBond<Array<Element.Item>>(createCell: createCell))
        }
    }
