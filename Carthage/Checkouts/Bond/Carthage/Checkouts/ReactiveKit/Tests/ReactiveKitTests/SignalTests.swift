...
class SignalTests: XCTestCase {
  ...
  func testPerformance() {
    self.measure {
       (0..<1000).forEach { _ in
        let signal = ReactiveKit.Signal<Int> { observer in
  ...
  func testProductionAndObservation() {
    ...
    let operation = Signal<Int>.sequence([1, 2, 3]).executeIn(bob.context)
  ...
  func testJust() {
    let operation = Signal<Int>.just(1)
    operation.expectComplete(after: [1])
  }

  func testSequence() {
    let operation = Signal<Int>.sequence([1, 2, 3])
    operation.expectComplete(after: [1, 2, 3])
  }

  func testCompleted() {
    let operation = Signal<Int>.completed()
    operation.expectComplete(after: [])
  }

  func testNever() {
    let operation = Signal<Int>.never()
    operation.expectNoEvent()
  }

  func testFailed() {
    let operation = Signal<Int>.failed(TestError.Error)
    operation.expect(events: [.failed(TestError.Error)])
  }
  ...
  func testBuffer() {
    let operation = Signal<Int>.sequence([1,2,3,4,5])
  ...
  func testDistinct() {
    let operation = Signal<Int>.sequence([1, 2, 2, 3])
...
//  }
  ...
  func testIgnoreNil() {
    let operation = Signal<Int?>.sequence(Array<Int?>([1, nil, 3]))
    let unwrapped = operation.ignoreNil()
    unwrapped.expectComplete(after: [1, 3])
  }

  func testReplaceNil() {
    let operation = Signal<Int?>.sequence(Array<Int?>([1, nil, 3, nil]))
  ...
  func testCombineLatestWith() {
    ...
    let operationA = Signal<Int>.sequence([1, 2, 3]).observeIn(bob.context)
    let operationB = Signal<String>.sequence(["A", "B", "C"]).observeIn(eve.context)
  ...
  func testMergeWith() {
    ...
    let eve = Scheduler()
    let operationA = Signal<Int>.sequence([1, 2, 3]).observeIn(bob.context)
    let operationB = Signal<Int>.sequence([4, 5, 6]).observeIn(eve.context)
  ...
  func testZipWith() {
    let operationA = Signal<Int>.sequence([1, 2, 3])
    let operationB = Signal<String>.sequence(["A", "B"])
    let combined = operationA.zip(with: operationB).map { "\($0)\($1)" }
    combined.expectComplete(after: ["1A", "2B"])
  }
  
  func testZipWithWhenNotComplete() {
    let operationA = Signal<Int>.sequence([1, 2, 3]).ignoreTerminal()
    let operationB = Signal<String>.sequence(["A", "B"])
    let combined = operationA.zip(with: operationB).map { "\($0)\($1)" }
    combined.expectComplete(after: ["1A", "2B"])
  }
  ...
  func testZipWithWhenNotComplete2() {
    let operationA = Signal<Int>.sequence([1, 2, 3])
    let operationB = Signal<String>.sequence(["A", "B"]).ignoreTerminal()
    let combined = operationA.zip(with: operationB).map { "\($0)\($1)" }
    combined.expect(events: [.next("1A"), .next("2B")])
  }
  
  func testZipWithAsyncSignal() {
    let operationA = Signal<Int>.interval(0.5).take(first: 4)  // Takes just 2 secs to emit 4 nexts.
    let operationB = Signal<Int>.interval(1.0).take(first: 10) // Takes 4 secs to emit 4 nexts.
  ...
  func testFlatMapError() {
    let operation = Signal<Int>.failed(TestError.Error)
    let recovered = operation.flatMapError { error in Signal<Int>.just(1) }
  ...
  func testRetry() {
    ...
    let operation = Signal<Int>.failed(TestError.Error).executeIn(bob.context)
    let retry = operation.retry(times: 3)
    retry.expect(events: [.failed(TestError.Error)])
  ...
  func testobserveIn() {
    ...
    let operation = Signal<Int>.sequence([1, 2, 3]).observeIn(bob.context)
    operation.expectComplete(after: [1, 2, 3])
  ...
  func testPausable() {
    let operation = PublishSubject<Int>()
    let controller = PublishSubject<Bool>()
  ...
  func testTimeoutNoFailure() {
    let exp = expectation(description: "completed")
    Signal<Int>.just(1).timeout(after: 0.2, with: TestError.Error, on: DispatchQueue.main).expectAsyncComplete(after: [1], expectation: exp)
    waitForExpectations(timeout: 1, handler: nil)
  }

  func testTimeoutFailure() {
    let exp = expectation(description: "completed")
    Signal<Int>.never().timeout(after: 0.5, with: TestError.Error, on: DispatchQueue.main).expectAsync(events: [.failed(TestError.Error)], expectation: exp)
    waitForExpectations(timeout: 1, handler: nil)
  }
  ...
  func testAmbWith() {
    ...
    let operationA = Signal<Int>.sequence([1, 2]).observeIn(bob.context)
    let operationB = Signal<Int>.sequence([3, 4]).observeIn(eve.context)
  ...
  func testDefaultIfEmpty() {
    let operation = Signal<Int>.sequence([])
  ...
  func testFlatMapMerge() {
    ...
    let operation = Signal<Int>.sequence([1, 2]).observeIn(bob.context)
    let merged = operation.flatMapMerge { num in
      return Signal<Int>.sequence([5, 6].map { $0 * num }).observeIn(eves[num-1].context)
    }
