...
extension SignalProtocol {
    ...
    public static func heartbeat(interval seconds: Double) -> Signal<Void> {
        ...
        return willEnterForeground.replace(with: ()).start(with: ()).flatMapLatest { () -> Signal<Void> in
            return Signal<Int>.interval(seconds, queue: .global()).replace(with: ()).start(with: ()).take(until: didEnterBackgorund)
        }
    }
